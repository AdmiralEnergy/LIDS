// Admiral Chat - API Routes
// Chat endpoints for team communication
// MVP: In-memory storage (same pattern as SMS inbound)
...
// Helper to get Twenty CRM token
...
router.get("/channels", async (req: Request, res: Response) => {
  try {
    ...
    // Get DM/private channels user is part of
...
// POST /api/chat/channels - Create channel or DM
...
router.get("/channels/:channelId/messages", async (req: Request, res: Response) => {
  try {
    ...
    // Limit and return in chronological order
...
router.post("/channels/:channelId/read", async (req: Request, res: Response) => {
  try {
    ...
    if (participation) {
      ...
      // Create participation for public channels
      participation = {
        id: `part-${uuid()}`,
        channelId,
...
// ============================================
// MEMBERS ENDPOINT (Proxy to Twenty CRM)
// ============================================

// GET /api/chat/members - Get workspace members
...
// ============================================
// SMS INTEGRATION (for inbound SMS → chat)
// ============================================

// POST /api/chat/sms/inbound - Route inbound SMS to chat
// Called by SMS webhook to add to #sms-inbox channel
...
// ============================================
// SEQUENCE NOTIFICATIONS (for n8n integration)
// ============================================

// POST /api/chat/sequence/notification - Add cadence notification to channel
router.post("/sequence/notification", async (req: Request, res: Response) => {
  try {
    ...
    const message: Message = {
      id: `msg-${uuid()}`,
      channelId: channelId || "ch-sales", // Default to sales channel
      senderId: "system",
...
// GET /api/chat/debug/stats - Get storage stats
